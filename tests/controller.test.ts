...
describe('VirtualOccupancySensorController', () => {
  ...
  describe('State: empty', () => {
    ...
    it('should transition to occupied on motion_detected', () => {
      controller.registerEvent('motion_detected', 'motion-1');
      expect(getLastState()).toBe('occupied');
    });

    it('should transition to door_open on any_door_open', () => {
      controller.registerEvent('any_door_open', 'door-1');
      expect(getLastState()).toBe('door_open');
    });

    it('should ignore all_doors_closed event', () => {
      controller.registerEvent('all_doors_closed', 'door-1');
      expect(stateChanges).toHaveLength(0);
    });

    it('should ignore motion_timeout event', () => {
      controller.registerEvent('motion_timeout', 'motion-1');
      expect(stateChanges).toHaveLength(0);
    });

    it('should ignore timeout event', () => {
      controller.registerEvent('timeout', 'system');
      expect(stateChanges).toHaveLength(0);
    });
  });
  ...
  describe('State: door_open', () => {
    ...
    it('should ignore any_door_open event (already in door_open)', () => {
      controller.registerEvent('any_door_open', 'door-2');
      expect(stateChanges).toHaveLength(0);
    });
  ...
  describe('Full State Machine Flows', () => {
    ...
    it('should handle quick exit flow: occupied -> door_open -> checking -> empty', () => {
      controller.registerEvent('motion_detected', 'motion-1'); // Get to occupied
      stateChanges = [];
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle rapid event sequences', () => {
      // Rapid door activity
      controller.registerEvent('any_door_open', 'door-1');
      controller.registerEvent('any_door_open', 'door-2'); // Already door_open
      controller.registerEvent('all_doors_closed', 'door-1');
      controller.registerEvent('all_doors_closed', 'door-2'); // Already checking

      expect(stateChanges).toEqual(['door_open', 'checking']);
    });

    it('should handle events from different devices correctly', () => {
      controller.registerEvent('any_door_open', 'front-door');
      expect(getLastState()).toBe('door_open');

      controller.registerEvent('motion_detected', 'living-room-sensor');
      // Motion is ignored in door_open state
      expect(getLastState()).toBe('door_open');

      controller.registerEvent('all_doors_closed', 'front-door');
      expect(getLastState()).toBe('checking');

      controller.registerEvent('motion_detected', 'bedroom-sensor');
      expect(getLastState()).toBe('occupied');
    });
  });
});
